...
Aplikasi ini menerima input berupa nama dan Nik Karyawan, dan memberikan output berupa informasi detail dari NPM tersebut seperti tahun masuk, fakultas, program studi, dan nomor registrasi.
...
```bash
mhs[i] = new KaryawanDetail(nama, nik);
```
...
8. **Inheritance** adalah konsep di mana sebuah class bisa mewarisi property dan method dari class lain. Pada kode ini, `KaryawanDetail` mewarisi `Mahasiswa` dengan sintaks `extends`.

```bash
public class MahasiswaDetail extends Karyawan {
    ...
}
```
...
13. **Array** adalah struktur data yang digunakan untuk menyimpan beberapa nilai dalam satu variabel. Pada kode ini, `KaryawanDetail[] Karyawan = new KaryawanDetail[2];` adalah contoh penggunaan array.
